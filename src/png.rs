...
use crate::{chunk::Chunk, Result, Error};
...
impl Png {
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        ...
        if let Some(chunk) = self.chunks.iter().find(|chunk| chunk.chunk_type().to_string() == chunk_type) {
            return Some(chunk);
        }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        ...
        bytes.append(
            &mut self.chunks.iter().flat_map(Chunk::as_bytes).collect()
        );
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        let (header, body) = value.split_at(8);

        let header = <[u8; 8]>::try_from(header).unwrap();

        match header == Png::STANDARD_HEADER {
            true => (),
            false => return Err("Invalid header".into())
        }
        ...
        loop {
            let length = u32::from_be_bytes(<[u8; 4]>::try_from(&body[0..4]).unwrap()) as usize;

            let (selected, rest) = body.split_at(length + 12);

            let new_chunk = Chunk::try_from(selected)?;
            ...
            match rest.len() {
                0 => break,
                x if x > 0 && x < 12 => return Err("Invalid byte input".into()),
                _ => continue
            }
